...
/** orbit centre = the model's head (nudged below the title) */
const CX = 50
const CY = 56
...
function ptDepth(f: Feature, ms: number) {
  ...
  const ex = f.rx * Math.cos(a)
  const ey = f.ry * Math.sin(a)
  return {
...
function FloatingBolt({ f }: { f: Feature }) {
  const t = useTime()
  const d = useTransform(t, (ms) => { const p = ptDepth(f, ms); return bolt(CX, CY, p.x, p.y, Math.floor(ms / 80), 2.4) })
  const off = useTransform(t, (ms) => (f.dir === 1 ? -1 : 1) * (ms / 45))
  const opacity = useTransform(t, (ms) => 0.25 + 0.4 * (ptDepth(f, ms).depth + 1) / 2)
  const c = `var(${f.hue})`
  return (
    <g>
...
function FloatingFeature({ f }: { f: Feature }) {
  const t = useTime()
  const left = useTransform(t, (ms) => `${ptDepth(f, ms).x}%`)
  const top = useTransform(t, (ms) => `${ptDepth(f, ms).y}%`)
  const scale = useTransform(t, (ms) => 0.72 + 0.4 * (ptDepth(f, ms).depth + 1) / 2)
  const opacity = useTransform(t, (ms) => 0.42 + 0.58 * (ptDepth(f, ms).depth + 1) / 2)
  const zIndex = useTransform(t, (ms) => (ptDepth(f, ms).depth > 0 ? 20 : 5))
  const labelOpacity = useTransform(t, (ms) => (ptDepth(f, ms).depth > 0.15 ? 1 : 0))
  const c = `var(${f.hue})`
  return (
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={() => { px.set(0); py.set(0) }} className="relative mx-auto h-full w-full max-w-3xl">
      <motion.div className="absolute inset-0" style={{ x: gx, y: gy }}>
        ...
        <div className="pointer-events-none absolute rounded-full" style={{ left: `${CX}%`, top: `${CY}%`, width: '52%', height: '70%', transform: 'translate(-50%,-50%)', zIndex: 2, background: 'radial-gradient(circle at 50% 44%, var(--bg-void) 0%, var(--bg-void) 38%, transparent 72%)' }} />
        ...
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" style={{ zIndex: 3 }}>
          {RINGS.map((o, i) => (
            <ellipse key={i} cx={CX} cy={CY} rx={o.rx} ry={o.ry} transform={`rotate(${o.rot} ${CX} ${CY})`} fill="none" stroke="var(--glass-border-strong)" strokeWidth="0.12" strokeOpacity="0.45" vectorEffect="non-scaling-stroke" />
          ))}
        </svg>
        ...
        <div
          ...
          style={{
            ...
            width: 'clamp(210px, 30vw, 320px)', height: 'clamp(270px, 40vw, 400px)',
          }}
        >
          <motion.div
        ...
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" style={{ zIndex: 11 }}>
          {FEATURES.map((f) => <FloatingBolt key={f.id} f={f} />)}
        </svg>

        {/* floating features (z toggles 5/20 → behind / in front of the head) */}
        {FEATURES.map((f) => <FloatingFeature key={f.id} f={f} />)}
      </motion.div>
    </div>
  )
}
